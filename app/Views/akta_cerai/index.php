...
<h3 class="mb-4">Layanan Akta Cerai</h3>

<?php if(session()->get('role') == 'desa' || session()->get('role') == 'admin'): ?>
<a href="<?= base_url('akta-cerai/create') ?>" class="btn btn-primary mb-3">
...
<a href="<?= base_url('akta-cerai') ?>" class="btn btn-light btn-sm">Reset</a>
...
<?php if($row['status'] == 'Pengajuan'): ?>
    <?php if(session()->get('role') == 'admin'): ?>
            <a href="<?= base_url('akta-cerai/detail/'.$row['id_permohonan']) ?>"
               class="btn btn-info btn-sm">
                Lihat
            </a>
    <?php endif; ?>

<a href="<?= base_url('akta-cerai/edit/'.$row['id_permohonan']) ?>" 
   class="btn btn-sm btn-warning">Edit</a>

<a href="<?= base_url('akta-cerai/delete/'.$row['id_permohonan']) ?>" 
...
<?php elseif($row['status'] == 'Selesai'): ?>
    ...
    <form action="<?= base_url('akta-cerai/pengembalian/'.$row['id_permohonan']) ?>" 
        method="post">
...
<form action="<?= base_url('akta-cerai/update-hasil/'.$row['id_permohonan']) ?>" 
      method="post" enctype="multipart/form-data"
      onsubmit="return confirmEditFile()">

<input type="file" name="file_hasil" class="form-control form-control-sm mb-1" required>
...
<div class="d-flex gap-1 flex-wrap mt-2">
    ...
    <!-- SETUJUI -->
    <form action="<?= base_url('akta-cerai/setujui-pengembalian/'.$row['id_permohonan']) ?>" 
          method="post">
...
<form action="<?= base_url('akta-cerai/tolak-pengembalian/'.$row['id_permohonan']) ?>" 
      method="post">
...
<?php endif; ?>
    ...
    <form action="<?= base_url('akta-cerai/update-status') ?>" 
        method="post" 
...
document.getElementById('kecamatan').addEventListener('change', function() {
    ...
    for (let option of desaSelect.options) {
        ...
        // tampilkan semua kalau kec kosong
